...
class SuratModel extends Model
{
    ...
    protected $fillable = [
        'id', 'uuid','id_user', 'nomor_surat', 'tanggal_surat', 'id_tahun', 'id_jenis_surat', 'file_surat', 'asal_surat','perihal', 'created_at', 'updated_at'
    ];
    ...
    {
        $data = $this->join('users', 'tb_surat_masuk.id_user', '=', 'users.id')
            ->select('users.uuid', 'users.name', 'users.email')
            ->where('tb_surat_masuk.id_user', '=', $id_user)
            ->first();
        return $data;
    }
}
